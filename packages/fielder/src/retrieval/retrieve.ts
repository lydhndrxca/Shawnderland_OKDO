...
export interface FielderRetrievalResult {
  corpusChunks: CorpusChunk[];
  decisionPatterns: DecisionEntry[];
  contextBlock: string;
}

export function isFielderDataLoaded(): boolean {
  return initialized;
}
...
export async function retrieveFielderContext(
  query: string,
  options?: {
    corpusTopK?: number;
    decisionTopK?: number;
...
): Promise<FielderRetrievalResult> {
