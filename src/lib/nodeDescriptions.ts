...
export const NODE_DESCRIPTIONS: Record<string, NodeDescription> = {
  ...
  propIdentity: {
    label: 'Prop Identity',
    color: '#009688',
    description: 'Define a prop\'s core identity — type (furniture, vehicle, weapon, etc.), setting/era, condition, and scale. These values shape the generation prompt.',
    connectsTo: ['Generate Prop'],
    connectsFrom: ['Extract Prop Attrs'],
  },
  propDescription: {
    label: 'Prop Description',
    color: '#5c6bc0',
    description: 'Write a freeform description of the prop. Use "Populate" to have AI analyze the text and fill connected Identity and Attributes nodes.',
    connectsTo: ['Generate Prop', 'Enhance Prop Desc'],
    connectsFrom: ['Extract Prop Attrs'],
  },
  propAttributes: {
    label: 'Prop Attributes',
    color: '#9c27b0',
    description: 'Detailed prop attributes — material, surface finish, wear patterns, color palette, functional elements, decorative details, and more. Each field can be randomized or locked.',
    connectsTo: ['Generate Prop'],
    connectsFrom: ['Extract Prop Attrs'],
  },
  propStyle: {
    label: 'Prop Style',
    color: '#7b1fa2',
    description: 'Set the visual style for prop renders — reference images, text description, or both. Influences the overall look and rendering approach.',
    connectsTo: ['Generate Prop'],
  },
  propGenerate: {
    label: 'Generate Prop',
    color: '#e91e63',
    description: 'Central prop generation engine. Gathers identity, description, attributes, and style inputs to generate prop images. Supports isolation mode — extract a specific subject from a reference image and render it on a neutral background. Sends results to all connected prop viewers.',
    connectsTo: ['Prop Main Stage', 'Prop Front View', 'Prop Back View', 'Prop Side View', 'Prop Top View'],
    connectsFrom: ['Prop Identity', 'Prop Description', 'Prop Attributes', 'Prop Style', 'Prop Ref Callout', 'Image Reference', 'Bulk Image Input'],
  },
  propExtractAttrs: {
    label: 'Extract Prop Attrs',
    color: '#ffab40',
    description: 'Feed an image in and AI analyzes it to automatically fill prop identity, description, and attribute fields. Identifies materials, wear patterns, functional elements, and decorative details.',
    connectsTo: ['Prop Identity', 'Prop Description', 'Prop Attributes'],
    connectsFrom: ['Image Reference', 'Image Influence', 'Any viewer node'],
  },
  propEnhanceDesc: {
    label: 'Enhance Prop Desc',
    color: '#66bb6a',
    description: 'AI expands and enriches a prop description with professional environment art terminology — specific material callouts, wear patterns, surface detail, and construction techniques.',
    connectsTo: ['Prop Description'],
  },
  propRefCallout: {
    label: 'Prop Ref Callout',
    color: '#26a69a',
    description: 'Annotate a reference image with specific callouts for the prop generator — e.g., "Use the handle design from this reference" or "Match the weathering pattern."',
    connectsTo: ['Generate Prop'],
    connectsFrom: ['Image Reference'],
  },
  propMainViewer: {
    label: 'Prop Main Stage',
    color: '#00bfa5',
    description: 'Primary prop image display — hero shot. Supports zoom, pan, inline editing, image history, and gallery navigation.',
    connectsTo: ['Image Studio', 'Export', 'Image → 3D'],
    connectsFrom: ['Generate Prop'],
  },
  propFrontViewer: {
    label: 'Prop Front View',
    color: '#42a5f5',
    description: 'Front orthographic view of the prop. Auto-generates when Main Stage produces an image.',
    connectsTo: ['Image Studio', 'Export', 'Image → 3D'],
    connectsFrom: ['Generate Prop'],
  },
  propBackViewer: {
    label: 'Prop Back View',
    color: '#ab47bc',
    description: 'Back orthographic view of the prop. Auto-generates when Main Stage produces an image.',
    connectsTo: ['Image Studio', 'Export', 'Image → 3D'],
    connectsFrom: ['Generate Prop'],
  },
  propSideViewer: {
    label: 'Prop Side View',
    color: '#ff7043',
    description: 'Side orthographic profile view of the prop. Auto-generates when Main Stage produces an image.',
    connectsTo: ['Image Studio', 'Export', 'Image → 3D'],
    connectsFrom: ['Generate Prop'],
  },
  propTopViewer: {
    label: 'Prop Top View',
    color: '#26a69a',
    description: 'Top-down orthographic view of the prop — unique to PropLab. Essential for environment artists who need plan views for props.',
    connectsTo: ['Image Studio', 'Export', 'Image → 3D'],
    connectsFrom: ['Generate Prop'],
  },
  ...
  uiConfig: {
    label: 'UI Config',
    color: '#1565c0',
    description: 'Set the generation intent (Freeform, HUD, Achievement, Weapon, ROMZ, Map Icons, HUD UI Layouts, etc.), resolution mode (Freeform, 16-bit, 64-bit pixel art), and wear level. For HUD UI Layouts, select which HUD elements to include. Displays extracted icon spec data when pushed from Extract Icon Spec.',
    connectsTo: ['Generate UI Asset'],
    connectsFrom: ['Extract Icon Spec (pushes spec back)'],
  },
  uiPrompt: {
    label: 'UI Prompt',
    color: '#7b1fa2',
    description: 'Write the prompt describing the UI asset you want. Use refA, refB, refC tokens to reference specific connected images. Detected tokens are shown as badges below the text.',
    connectsTo: ['Generate UI Asset'],
  },
  uiStyle: {
    label: 'UI Style',
    color: '#6a1b9a',
    description: 'Upload or paste style reference images — the AI will emulate their artistic DNA (line weight, color palette, pixel density, texture). Switch between Images Only, Text Only, or Images + Text modes. Process images to generate a text style description.',
    connectsTo: ['Generate UI Asset'],
  },
  uiGenerate: {
    label: 'Generate UI Asset',
    color: '#e91e63',
    description: 'Central generation engine. Gathers config, prompt, style images, and labeled references. Supports batch generation (1-10 concurrent images). Handles special modes: ROMZ (landscape, no chroma), Map Icons (B&W), HUD UI (layout mockups). Pushes results to downstream viewers.',
    connectsTo: ['UI Asset Viewer'],
    connectsFrom: ['UI Config', 'UI Prompt', 'UI Style', 'Image Influence (RefA/B/C)'],
  },
  uiMainViewer: {
    label: 'UI Asset Viewer',
    color: '#1565c0',
    description: 'Primary output viewer for generated UI assets. Gallery navigation for batch results, inline text editing, image history with prompt recall. Open or paste images directly. Connect to Finalize, Export, or Extract nodes.',
    connectsTo: ['Finalize (Chroma Key)', 'Export', 'Extract Icon Spec'],
    connectsFrom: ['Generate UI Asset'],
  },
  uiExtractSpec: {
    label: 'Extract Icon Spec',
    color: '#ff6f00',
    description: 'AI analyzes an icon image and extracts a structured JSON spec — shape language, material, color palette, outline style, shading, rendering medium, and more. Pushes the extracted spec to a connected UI Config node.',
    connectsTo: ['UI Config'],
    connectsFrom: ['UI Asset Viewer', 'Image Influence'],
  },
  uiFinalize: {
    label: 'Finalize (Chroma Key)',
    color: '#00695c',
    description: 'Client-side image processing pipeline. Chroma Green mode removes #00FF00 backgrounds via flood-fill + alpha keying. B&W Threshold mode converts to pure black and white (for map icons). Shrink Border removes 1px edge fringe. Full undo stack.',
    connectsTo: ['Export'],
    connectsFrom: ['UI Asset Viewer'],
  },
};
