...
export interface SerlingRetrievalResult {
  corpusChunks: CorpusChunk[];
  decisionPatterns: DecisionEntry[];
  contextBlock: string;
}

export function isSerlingDataLoaded(): boolean {
  return initialized;
}
...
export async function retrieveSerlingContext(
  query: string,
  options?: {
    corpusTopK?: number;
    decisionTopK?: number;
...
): Promise<SerlingRetrievalResult> {
