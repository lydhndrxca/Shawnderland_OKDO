...
function UIStyleNodeInner({ id, data, selected }: Props) {
  ...
  const handleProcessStyle = useCallback(async () => {
    ...
    try {
      const prompt = `You are a visual style analyst specializing in game UI and icon art. Study the provided reference image(s) and produce a detailed style description that captures the artistic DNA for recreating this visual style in UI asset generation.

Analyze: color palette, outline weight, pixel density, shading technique, texture quality, overall aesthetic era (e.g., 16-bit, modern vector, hand-painted).
  ...
  return (
    <div className={`style-glass-node ${selected ? 'selected' : ''}`}>
      <div className="style-glass-header" style={{ cursor: 'pointer' }} onClick={() => setCollapsed((c) => !c)}>
        <span className="style-glass-title">UI Style</span>
      ...
      {!collapsed && (
        <div className="style-glass-body">
          <div className="style-glass-grid-area nodrag nowheel">
            {images.length === 0 && <div className="style-glass-empty">No style images yet</div>}
            {images.map((img, idx) => (
              <div key={idx} className="style-glass-thumb" onClick={() => setExpandedIdx(expandedIdx === idx ? null : idx)}>
                <img src={`data:${img.mimeType};base64,${img.base64}`} alt={`Style ${idx + 1}`} draggable={false} />
...
export default memo(UIStyleNodeInner);
