...
function UIMainViewerInner({ id, data, selected }: Props) {
  ...
  const currentImage = editedImage ?? pushedImage ?? upstreamImage ?? localImage;
  ...
  useEffect(() => {
    if (upstreamImage) pushHistory(upstreamImage, 'Generated asset');
  }, [upstreamImage, pushHistory]);
  ...
  const [editModel, setEditModel] = useState<GeminiImageModel>((data?.editModel as GeminiImageModel) ?? 'gemini-flash-image');
  ...
  const handleEdit = useCallback(async () => {
    ...
    try {
      const prompt = `VISUAL EDIT TASK:\nPreserve 100% of the existing design, only apply:\n${editText.trim()}\nApply ONLY the above modification. Do NOT change anything else.\nKeep chroma green background if present.\nZERO TEXT IN IMAGE: Do NOT render any text, labels, letters, numbers, or watermarks anywhere in the image.`;
      const results = await generateWithGeminiRef(prompt, [srcImage], editModel, '1:1');
  ...
  }, [viewImage, editText, editBusy, id, setNodes, pushHistory, editModel]);

  // Generate / regenerate
  const [genBusy, setGenBusy] = useState(false);
  const handleRegenerate = useCallback(async () => {
    if (genBusy || !viewImage) return;
    setGenBusy(true);
    setEditError(null);
    setEditElapsed(0);
    const t0 = Date.now();
    ...
    try {
      ...
      const editInstr = editText.trim() || 'Regenerate this UI asset with the same design.';
      const prompt = `VISUAL EDIT TASK:\n${editInstr}\nKeep chroma green background if present.\nZERO TEXT IN IMAGE: Do NOT render any text, labels, letters, numbers, or watermarks.`;
      const promises = Array.from({ length: count }, () => generateWithGeminiRef(prompt, [viewImage], editModel, '1:1'));
      ...
      setGalleryIdx(0);
      for (const img of gallery) pushHistory(img, 'Regen');
    } catch (e) {
      const msg = e instanceof Error ? e.message : String(e);
      if (!msg.toLowerCase().includes('cancel') && !msg.toLowerCase().includes('abort')) {
        if (mountedRef.current) setEditError(msg);
  ...
  }, [genBusy, genCount, viewImage, editText, id, setNodes, pushHistory, editModel]);

  const externalGenerating = (data?.generating as boolean) ?? false;
  const anyBusy = editBusy || genBusy || externalGenerating;
    ...
    >
      ...
      {showFullPreview && viewImage && (
        <div className="style-popup-overlay nodrag" onClick={(e) => e.stopPropagation()}>
          <div className="style-popup-window" style={{ maxWidth: 520, maxHeight: 620 }}>
            ...
            <img src={`data:${viewImage.mimeType};base64,${viewImage.base64}`} alt="UI Asset" draggable={false} style={{ maxWidth: 500, maxHeight: 580, objectFit: 'contain', display: 'block', borderRadius: 4 }} />
          </div>
        </div>
      )}
      ...
      {!compact && (
        <div className="char-viewer-canvas nodrag nowheel" style={{ flex: 1, overflow: 'hidden' }}>
          {viewImage ? (
            <>
              <ImageContextMenu image={viewImage} alt="UI Asset" onPasteImage={handlePasteImage}>
                <img
            ...
            <span className="char-viewer-empty">No image loaded<br />Generate or open a file</span>
          )}
        </div>
      )}
        ...
        <>
          ...
          <div className="char-viewer-toolbar" style={{ justifyContent: 'center' }}>
            <button
              ...
              disabled={anyBusy || !viewImage}
              style={{ background: anyBusy ? undefined : '#1565c0', color: '#fff', fontWeight: 600, flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 3 }}
            >
              {genBusy ? `${editElapsed}s\u2026` : (
                <>
                  ...
                  <input
                    ...
                    style={{ width: 28, textAlign: 'center', padding: '0 2px', fontSize: 11, fontWeight: 700, background: 'rgba(0,0,0,0.2)', border: '1px solid rgba(0,0,0,0.3)', borderRadius: 3, color: '#fff', outline: 'none' }}
                  />
                  {' '}Image{genCount !== 1 ? 's' : ''}
                </>
              )}
      ...
      {!compact && viewImage && (
        <div style={{ padding: '4px 6px 6px', borderTop: '1px solid #333' }} className="nodrag">
          <textarea
            ...
            placeholder="Edit asset (e.g., change color palette, add grit)..."
            disabled={anyBusy}
            rows={3}
          ...
          <div style={{ display: 'flex', gap: 4, alignItems: 'center', marginTop: 4 }}>
            <select className="nowheel" value={editModel} disabled={anyBusy}
              ...
              {MULTIMODAL_MODELS.map((m) => (
                <option key={m.apiId} value={m.apiId}>{m.label.replace(/[()]/g, '').split(' ').slice(0, 2).join(' ')}</option>
              ))}
            </select>
...
export default memo(UIMainViewerInner);
